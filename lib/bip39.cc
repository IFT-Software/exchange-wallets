...
const std::string BIP39::GetSeedStr() {
...
const std::bitset<4> BIP39::GenerateChecksum() {
  int checksum_size =
      (entropy_).size() / 32;  // 1 bit checksum for every 32 bits of entropy
...
const std::array<uint8_t, 64> BIP39::GenerateSeed() {
  ...
  Crypto::PBKDF2_HMAC_SHA512(this->GetMnemonic().c_str(),
                             (uint8_t*)salt.c_str(), salt.size(), iterations,
                             keylength, seed_.begin());
  return seed_;
}
...
const std::array<uint8_t, 64> BIP39::GenerateSeedFromEntropy(
